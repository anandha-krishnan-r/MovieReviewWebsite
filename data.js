/**
 * data.js
 * CineSphere Movie Recommendation Database & Persistence Layer
 * Curated list of 18 classic/popular movies with full metadata, critic reviews, and platforms.
 * Manages user custom reviews and dynamic ratings persistence via localStorage.
 */
...
/**
 * Persisted state helper functions.
 * Merges localStorage custom reviews and ratings into our core dataset.
 */
class MovieStore {
  constructor() {
  ...
  // Load movies from localStorage or initialize from pre-curated list
  _loadMovies() {
    const localData = localStorage.getItem("cinesphere_movies");
    if (!localData) {
      // Initialize localStorage with initial data
      localStorage.setItem("cinesphere_movies", JSON.stringify(INITIAL_MOVIES));
      localStorage.setItem("cinesphere_user_reviews", JSON.stringify({}));
      localStorage.setItem("cinesphere_user_ratings", JSON.stringify({}));
      return JSON.parse(JSON.stringify(INITIAL_MOVIES));
    }

    const baseMovies = JSON.parse(localData);
    const userReviews = JSON.parse(localStorage.getItem("cinesphere_user_reviews") || "{}");
    const userRatings = JSON.parse(localStorage.getItem("cinesphere_user_ratings") || "{}");

    // Merge custom ratings and reviews into standard list
    return baseMovies.map(movie => {
      const movieCopy = { ...movie };
      
      // Merge user reviews (append to critic reviews under "User Review" tag or separate array)
      if (userReviews[movie.id]) {
        movieCopy.userReviews = userReviews[movie.id];
      } else {
        movieCopy.userReviews = [];
      }

      // Merge user rating and calculate dynamic new rating averages
      if (userRatings[movie.id]) {
        const ratingsArray = userRatings[movie.id]; // array of star numbers (1-10)
        const totalUserRating = ratingsArray.reduce((acc, score) => acc + score, 0);
        const userAvg = Number((totalUserRating / ratingsArray.length).toFixed(1));
        
        movieCopy.userAverageRating = userAvg;
        movieCopy.userRatingCount = ratingsArray.length;
        
        // Recalculate combined IMDb display rating if there are user ratings
        movieCopy.adjustedImdbRating = Number(((movie.imdbRating * 10 + userAvg * ratingsArray.length) / (10 + ratingsArray.length)).toFixed(1));
      } else {
        movieCopy.userAverageRating = null;
        movieCopy.userRatingCount = 0;
        movieCopy.adjustedImdbRating = movie.imdbRating;
      }
      return movieCopy;
    });
  }
  ...
  // Retrieve movies categorized by language
  getMoviesByLanguage(language) {
    return this.movies.filter(movie => movie.language.toLowerCase() === language.toLowerCase());
  }

  // Retrieve movies categorized by film industry
  getMoviesByIndustry(industry) {
    return this.movies.filter(movie => movie.industry.toLowerCase() === industry.toLowerCase());
  }

  // Retrieve weekly trending movies
  getWeeklyTrending() {
    return this.movies.filter(movie => movie.trendingThisWeek);
  }

  // Get unique languages present
  getLanguages() {
    const langs = this.movies.map(movie => movie.language);
    return [...new Set(langs)].sort();
  }

  // Get unique industries present
  getIndustries() {
    const inds = this.movies.map(movie => movie.industry);
    return [...new Set(inds)].sort();
  }

  // Perform search across title, synopsis, genres, language, and industry
  searchMovies(query) {
    if (!query) return [];
  ...
  addRating(movieId, stars) {
    ...
    userRatings[movieId].push(ratingScore);
    localStorage.setItem("cinesphere_user_ratings", JSON.stringify(userRatings));
  ...
  addReview(movieId, authorName, reviewContent, stars) {
    const cleanAuthor = authorName.trim() || "Anonymous Film Critic";
    ...
    const ratingScore = Math.max(1, Math.min(10, Number(stars)));
    const newReview = {
      criticName: cleanAuthor,
      publication: "CineSphere Community",
      score: ratingScore * 10, // scale rating up to percentage out of 100 for review styling consistency
      quote: cleanContent,
      date: new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
    };

    userReviews[movieId].unshift(newReview); // add to top
    localStorage.setItem("cinesphere_user_reviews", JSON.stringify(userReviews));

    // Automatically add rating as well
    this.addRating(movieId, stars);
...
// Instantiate and export global store instance
window.movieStore = new MovieStore();
